...
def getScreenshot(renderView1, dstdir, side, phi, theta, is_yA32):
    x = 0
    y = 900000
    z = 0
    r = side
...
def get_view(srcfile_path, filename, dstdir, side, channel, tf, view_size, phi, theta, is_yA32):
    ...
    if phi == -1:
        ...
        renderView1.CameraParallelScale = 1600000
    else:
        ...
        getScreenshot(renderView1, dstdir, side, phi, theta, is_yA32)
...
def get_rendering_result():
    ...
    while True:
        if queue_image_cnt <= 0:
            time.sleep(0.01)
            continue

        str_in = image_path_queue.get()
        ...
        if os.path.exists(directory + volume_dir):
            ...
            if phi == -1:
                theta = 0
            file_name = simulation + '_' + resolution + '_' + scalar + '_' + channel + '_' + timestep + '_' + str(phi) + '_' + str(theta)
            ...
            get_view(srcfile_path, file_name, dst_dir, volume_size, channel, preset_tf, view_size, phi, theta, is_yA32)
